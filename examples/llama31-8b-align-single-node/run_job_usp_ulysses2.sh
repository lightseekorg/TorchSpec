...
RAY_TEMP_DIR="${RAY_TEMP_DIR:-$RAY_TMP_ROOT/ray_ts_l31_usp_u2_$(id -u)}"
...
RAY_LOG_ROOT="$WORKING_DIR/tmp/ray_usp_u2_${DATE_TAG}_${LOG_SUFFIX}"
...
TENSORBOARD_LOGDIR="${TENSORBOARD_LOGDIR:-$ROOT_DIR/outputs/llama31_8b_align/tensorboard}"
mkdir -p "$LOG_DIR"
mkdir -p "$TENSORBOARD_LOGDIR"

TIMESTAMP="$(date +"%Y%m%d_%H%M%S")"
LOG_FILE="$LOG_DIR/llama31_8b_align_usp_ulysses2_${TIMESTAMP}.log"
...
if [ "${DISTRIBUTED_NODE_RANK:-0}" -eq 0 ]; then
    ...
    echo "=============================================="
    echo "TorchSpec Llama-3.1-8B USP alignment run"
    echo "=============================================="
