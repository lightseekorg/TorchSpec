...
def _last_hidden_states(cfg):
    torch.manual_seed(456)
    return torch.randn(B, SEQ, cfg.target_hidden_size, device=DEV, dtype=DT)
...
def test_training_args_override_domino_head_config():
    cfg = _cfg()
    trainer = object.__new__(DominoTrainer)
    trainer.args = Namespace(domino_gru_hidden_size=12, domino_correction_rank=5)

    draft = DominoTrainer._build_draft_model(trainer, cfg)

    assert draft.gru_hidden_size == 12
    assert draft.correction_rank == 5
    assert draft.causal_gru.hidden_size == 12
    assert draft.correction_w1.in_features == cfg.hidden_size + 12
    assert draft.correction_w1.out_features == 5
    assert draft.correction_w2.in_features == 5
...
def test_trainer_forward_preserves_loss_components_slot(monkeypatch):
    ...
    batch = {
        ...
        "loss_mask": loss_mask,
        "last_hidden_states": _last_hidden_states(cfg),
    }
...
def test_ce_loss_alpha_scales_domino_curriculum_loss():
    cfg = _cfg()
    batch = _batch(cfg)
    torch.manual_seed(7)
    base_model = _build(cfg, loss_decay_gamma=None)
    scaled_model = _build(cfg, loss_decay_gamma=None, ce_loss_alpha=0.25)
    scaled_model.load_state_dict(base_model.state_dict())

    torch.manual_seed(11)
    base_loss = _fwd(base_model, batch, 0.3)[0]
    torch.manual_seed(11)
    scaled_loss = _fwd(scaled_model, batch, 0.3)[0]

    torch.testing.assert_close(scaled_loss, base_loss * 0.25)


def test_l1_loss_requires_last_hidden_states():
    cfg = _cfg()
    model = _build(cfg, l1_loss_alpha=0.5)

    with pytest.raises(ValueError, match="last_hidden_states"):
        _fwd(model, _batch(cfg), 0.0)


def test_l1_loss_accepts_last_hidden_states():
    cfg = _cfg()
    model = _build(cfg, l1_loss_alpha=0.5)
    batch = _batch(cfg)
    model.curriculum_lambda = 0.0
